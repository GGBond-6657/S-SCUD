...
class ChatChain:
    ...
    def __init__(self,
        ...
        self.solidity_path = solidity_path
        ...
        if solidity_path:
            self.chat_env.env_dict['solidity_file'] = solidity_path
    ...
    def post_processing(self):
        ...
        # 导出二分类结果到CSV
        self._export_binary_classification_result()
    ...
    def _export_binary_classification_result(self):
        """导出二分类结果到CSV文件"""
        import csv
        from pathlib import Path
        
        # 检查是否有二分类结果
        result = self.chat_env.env_dict.get('binary_classification_result')
        if not result:
            return
        
        # 获取文件名（包括上级目录）
        sol_file = result.get('solidity_file', '')
        if sol_file:
            sol_path = Path(sol_file)
            # 提取 "上级目录名/文件名" 格式
            if len(sol_path.parts) >= 2:
                file_name = f"{sol_path.parts[-2]}/{sol_path.name}"
            else:
        ...
        llm_probing_executed = result.get('llm_probing_executed', True)
        
        # 根据是否执行LLM探测决定标签
        if has_vulnerability is None or not llm_probing_executed:
            vulnerability_label = 'NA'  # 未执行LLM探测
        else:
            vulnerability_label = 'Yes' if has_vulnerability else 'No'
        
        # CSV文件路径（保存在软件目录下）
        csv_path = os.path.join(self.chat_env.env_dict['directory'], 'binary_classification_result.csv')
        
        # 写入CSV
        with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['File Name', 'Has Vulnerability'])
            writer.writerow([file_name, vulnerability_label])
        
        log_visualize(f"**[Binary Classification Result Exported]**\nFile: {csv_path}\nResult: {file_name} -> {vulnerability_label}")
