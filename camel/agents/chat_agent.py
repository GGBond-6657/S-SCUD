...
class ChatAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.model: ModelType = (model if model is not None else ModelType.GPT_4_O_MINI)
    ...
    def step(
        ...
        if num_tokens < self.model_token_limit:
            ...
            if output_messages and len(output_messages) > 0:
                ...
                print(f"Warning: Received empty response from API. Response info: {info}")
                output_messages = [
                    ChatMessage(role_name=self.role_name, role_type=self.role_type,
                               meta_dict=dict(), role="assistant",
                               content="Error: Received empty response from the API. Please try again.")
                ]
        else:
